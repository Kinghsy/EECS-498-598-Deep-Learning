...
img_save_dir = "./Q4_figs/"
img_extension = ".png"
img_save_index = 0


def show_images(images):
    global img_save_index, img_extension, img_save_dir
    ...
    plt.savefig(img_save_dir + img_save_index.__str__() + img_extension)
    img_save_index += 1
    return 
...
def bce_loss(input, target):
    ...
    ###########################
    #loss = None
    loss_p1 = input.clamp(min=0)
    loss_p2 = -input * target
    loss_p3 = torch.log(1+torch.exp(-torch.abs(input)))
...
def discriminator_loss(logits_real, logits_fake, dtype):
    ...
    labels = torch.ones(logits_real.size()).type(dtype)
    loss_real = bce_loss(logits_real, labels)
    loss_fake = bce_loss(logits_fake, 1-labels)
...
def generator_loss(logits_fake, dtype):
    ...
    ######### TO DO ###########
    ###########################
    labels = torch.ones(logits_fake.size()).type(dtype)
    loss = bce_loss(logits_fake, labels).type(dtype)
    return loss
...
def main():
    ...
    run_a_gan(D_DC, G_DC, D_DC_solver, G_DC_solver, discriminator_loss,
              generator_loss, loader_train, dtype, num_epochs=20)
