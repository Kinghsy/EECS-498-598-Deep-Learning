...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        style_loss += style_weights[i] * torch.sum((current - style_targets[i]) ** 2)
    return style_loss
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        loss = content_loss(content_weight, feats[content_layer], content_target) + style_loss(feats, style_layers, style_targets, style_weights) + tv_loss(img_var, tv_weight)
        loss.backward()
        losses.append(loss)
    ...
    plt.imshow(deprocess(img.cpu()))
    plt.show()
    plt.savefig(content_image[7:-4]+'+'+style_image[7:-4]+'.png')
    plt.close()
    ...
    plt.ylabel('loss')
    plt.show()
    plt.savefig(content_image[7:-4]+'+'+style_image[7:-4]+'_loss.png')
    plt.close()
