...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    da = np.hstack([ f * (1 - f) * df, i * (1 - i) * di, (1 - g * g) * dg, o * (1 - o) * do,])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for ti in range(T):
        h[ti, :, :], c_temp, cache[ti] = lstm_step_forward(x[ti, :, :], prev_h, prev_c, Wx, Wh, b)
        prev_c = c_temp
        prev_h = h[ti, :, :]
...
def lstm_backward(dh, cache):
    ...
    for ti in range(T - 1, -1, -1):
        dx[ti, :, :], dprev_h, dprev_c, dWx_tmp, dWh_tmp, db_tmp = lstm_step_backward(dh[ti, :, :] + dprev_h, dprev_c, cache[ti])
        dWx += dWx_tmp
        dWh += dWh_tmp
        db += db_tmp
...
def temporal_fc_forward(x, w, b):
    ...
    N, T, D = x.shape
    _, M = w.shape
    out = np.reshape(np.dot(np.reshape(x, (N*T, D)), w) + b, (N, T, M))
    cache = (x, w, b, out)
...
def temporal_fc_backward(dout, cache):
    ...
    N, T, M = dout.shape
    D, _ = w.shape
    dout_t = np.reshape(dout, (N*T, M))
    dx = np.reshape(np.dot(dout_t, w.T) , (N, T, D))
    dw = np.dot(np.reshape(x, (N*T, D)).T, dout_t)
    db = np.sum(dout_t, axis = 0)
